...
function[poa,fstar,optPoa] = BlindUtility_design_function(n,k,f,w,printing)
    ...
    if k>n
        error('Blind agents more than total agents')
    end
    ...
    if printing ==1
        fprintf('\n--------------------------------------------------------\n')
        fprintf('Total agents = %d (%d normal, %d Blind)',n, n-k, k)
        fprintf('\n--------------------------------------------------------')
    ...
    [xval,poa]=  BlindComputingPoA(n,f,w, k, platform);
    ...
    pr_vf = 0;
    if pr_vf == 1
        [ppoa] = BlindPrimalLP_PoA(f,w,n,k,platform);
        fprintf('\n The price of anarchy for chosen mechanism is (primal) = %f',ppoa)
        fprintf('\n--------------------------------------------------------')
    end

    %% Linear program to give best possible bound on price of anarchy
 
       
    [fval,optPoa] =  BlindOptimizingPoA(n,w,k,platform);
    fstar = fval(2:n+1)';
