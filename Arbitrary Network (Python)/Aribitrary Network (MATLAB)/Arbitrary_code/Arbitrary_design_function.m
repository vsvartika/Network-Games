...
function[fstar,optPoa,poa] = Arbitrary_design_function(C,N,F,w,d,printing)
...
%% Linear program to *Optimize* price of anarchy     
    ...
    if printing ==1
        ...
        fprintf('------------------------------------------------------------------------------------------------------------------------------------------')
        if d>=1
        fprintf('\n The difference in PoA of chosen mechanism and optimal mechanism is %f', optPoa-poa)
        fprintf('\n The conjectured optimal PoA (only for supermodular w) is %f \n differnce between conj and actual =%f',n/w(n), abs(optPoa-n/w(n)))
        end
        fprintf('\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> \n')
    end
end
